...
> **Status: P3 완료** — P0 ✅ · P1 ✅(측정①) · P2 ✅(측정②) · P3 적재 ✅(Qdrant `ok`, points>0). 다음 = **P4 검색 + 정밀도 게이트**.
> 코드: `src/`(+ Indexer/Qdrant 적재) · `examples/bank_sample/` · `scripts/`(eval_extract·build_manuals·load_manuals) · `tests/`(8) · `data/manuals/` · `.venv/`.
> 실행: `docker compose up -d`(Qdrant 6335) → `uvicorn src.main:app --port 9000`.
> 빌드·적재: `python scripts/build_manuals.py` → `python scripts/load_manuals.py`. 측정: `scripts/eval_extract.py` · `pytest`.
