...
> **Status: P2 완료** — P0 ✅ · P1 Extractor ✅(측정①) · P2 매뉴얼 생성 ✅(측정②). 다음 = **P3 적재**.
> 코드: `src/`(엔진 골격 + Extractor + ManualBuilder) · `examples/bank_sample/` · `tests/`(8 pass) · `data/manuals/`(생성물) · `.venv/`.
> 실행: `docker compose up -d`(Qdrant 6335) → `uvicorn src.main:app --port 9000`.
> 빌드: `PYTHONPATH=. python scripts/build_manuals.py [--no-llm|--freeze]`. 측정: `scripts/eval_extract.py` · `pytest`.
...
**[프로토타입 데이터 형식 — 확정]** 현업 실제 형태는 미확인이라, 프로토타입 데이터는 아래로 직접 정한다.
실 환경(MariaDB `information_schema` + `COMMENT`)과 동형이라 전환 시 추출 로직을 그대로 쓴다.
...
feasibility는 한 곳에 몰려 있다 — **Extractor가 실 코드에서 lineage를 뽑는 정확도.**
나머지(매뉴얼 생성·게이트·핸드오프·검색 재사용)는 표준 엔지니어링.

핵심 미지수 3개 (실 코드/스키마를 봐야 확정):
1. **자체 프레임워크가 lineage를 얼마나 숨기나** (표준 Spring/MyBatis 대비) → 어댑터 공수.
2. **컬럼·코드값 한글 커버리지** — 프로토타입은 DDL `COMMENT`로 직접 채워 통제. *현업*에 같은 커버리지가 있는지는 미확인 → 표기의미 정밀도.
3. **SQL/매퍼가 정적이냐 동적이냐**(`<if>` 등) → lineage 정밀도.

### 검증 슬라이스 (전면 구현 전 가장 싸게 리스크를 깬다)

**실 코드로 단 하나의 화면을 세로로 끝까지** 돌려, 3가지 측정:
1. 변환기가 그 체인(화면→…→테이블)을 **정확히** 뽑는가? ← A 전체의 운명.
2. 생성된 매뉴얼이 **개발자 검토를 통과**하는가?
3. 그 화면 실제 담당자 질문 중 **몇 %를** 받아들일 정밀도로 답하는가?

깨끗한 샘플은 1번을 알려주지 못한다. **실 코드 1개 화면이 사업성을 판가름한다.**
A는 좁게 시작해 자라는 구조라 첫날 전체 커버리지가 필요 없다.
...
- **P3 · 적재**
  - 동결 매뉴얼 → KURE 임베딩 → Qdrant(payload: `table_en`/`table_ko`/`screen_id`/`role`…). lineage → 그래프.

- **P4 · 질의 + 정밀도 게이트**
  - retriever(하이브리드: dense+sparse+payload exact) → **정밀도 게이트**(점수 충분?) → 충분: 승인 매뉴얼 다듬어 답(generator) / 부족: 회피·핸드오프(뼈대).
  - `BNK_Bot` chat 도메인 *방식* 참고해 새로 구현 + 게이트 추가.
  - **게이트(측정③):** 슬라이스 화면 질문의 응답률·정밀도.
...
- **P6 · 일반화** — Extractor 프레임워크 어댑터(자체 프레임워크 magic)·동적 SQL 대응, 화면 N개로 확대.
