...
- **P1 · Extractor (가장 큰 리스크)**
  - 코드/SQL/DDL 파서 → 화면→API→Service→Mapper→WHERE→테이블 **lineage** + 표기의미(`table_ko`/`column_ko`/코드값) + 오류 + **한글 주석(위치에 묶어)** 추출.
  - 출력 = 구조화된 "사실". lineage는 그래프 저장소(SQLite).
  - **게이트(측정①):** 슬라이스 화면의 체인을 정확히 추출하는가.

- **P2 · 매뉴얼 생성 + 검수·동결**
  - Qwen이 추출된 사실을 요약 → branch/it 두 버전. 숫자·조건 verbatim, 의도는 주석, 라벨은 `table_ko`.
  - 검수: 프로토타입은 파일/CLI 승인 → 동결.
  - **게이트(측정②):** 생성 매뉴얼이 개발자 검토를 통과하는가.
