...
> **Status: 설계 단계 (코드 없음).** 이 문서는 합의된 목표·설계·기술결정을 담는다.
> 형제 프로젝트: `../BNK_Bot`(원본, 약관/설명서 PDF RAG — 재사용 대상) · `../BNK_Bot_S`(소스-aware 프로토타입 — 참고 대상).
...
"매뉴얼을 만드는 저자"가 아니라 **"맞는 매뉴얼을 찾아주는 사서"**로 재사용한다.
...
**`ingestion`의 입력만 PDF→소스코드로 바꾸고, 척추(chunk→embed→qdrant)와 `chat` 도메인은 거의 그대로 가져온다.**

| 구분 | `BNK_Bot` 모듈 | 처리 |
|---|---|---|
| **그대로 재사용** | `qdrant.py` · `generator.py`(Qwen) · `embedder.py`(KURE) · main lifespan · `/health` | 인프라 동일 |
| **약간 손봐 재사용** | `retriever.py` · `service.py` · `router.py` | 컬렉션을 "매뉴얼"로, payload에 screen_id/table 추가, **정밀도 게이트 + 핸드오프 분기** 추가 |
| **신규 제작** | `ingestion` 입력부 = **소스코드 Extractor** | PDF 파서 자리에 lineage 추출기 |
| **신규 제작** | 매뉴얼 **생성·검토·동결**, **핸드오프 축적 루프** | `BNK_Bot`엔 없음(생성이 질의시점이라) |

### `BNK_Bot`에서 물려받는 것 (이미 A의 질의 절반)

`BNK_Bot`의 `chat` 도메인은 A의 질의 타임 불변식을 이미 일부 구현해 둠 — 새로 만들지 말고 물려받는다:
...
## 🔁 재사용 스탠스 (`BNK_Bot` 코드/자산을 어떻게 가져오나)

재사용은 한 덩어리가 아니다. 층위마다 가치·결합위험이 다르다 — **위로 갈수록 가치↑·위험↓.**

| 종류 | 가치 | 결합위험 | 방침 |
|---|---|---|---|
| **결정·교훈** (num_ctx, reasoning_effort, NFC, 회피 보정, 결정론 우선) | 매우 높음 | 0 | **무조건 가져온다** |
| **패턴·아키텍처** (도메인 분리, lifespan 싱글톤, payload, eval 루프) | 높음 | 0 | **무조건 가져온다** |
| **코드 copy-fork** (작고 안 변하는 어댑터를 *복사*해 독립 진화) | 중간 | 낮음 | **선별 복사** |
| **공유 라이브러리/모노레포** (두 프로젝트가 같은 모듈 import) | 중간 | 높음 | ❌ **안 한다** |

"아예 다른 프로젝트"라는 사실이 깎는 건 **맨 아래 한 줄뿐** — 공유 라이브러리 결합(= Qdrant 컨테이너를 가른 이유와 동일). 나머지 세 층위는 다른 프로젝트여도 가치가 안 줄어든다.

**두 규율:**
- **가장 큰 이득은 코드 줄이 아니라 "풀어둔 문제를 다시 안 푸는 것".** `BNK_Bot`의 "왜 이렇게 했나" 결정들을 버리고 클린 슬레이트로 가는 게 진짜 낭비.
- **결론이 아니라 이유를 재사용한다.** 예: `BNK_Bot`의 "단일 컬렉션"은 *"같은 임베딩·격리 불필요"* 전제에서 나왔고, 우리는 그 전제를 안 만족 → 별도 컨테이너로 벗어남. 재사용 = 추론(언제 나누나) 계승.

**모듈별 적용:**
- **copy-fork(헤드스타트, 거의 안 변함):** `qdrant.py`(컬렉션·하이브리드만 손봄) · `embedder.py`(KURE 그대로) · `generator.py`(Ollama/Qwen 어댑터) · main lifespan 골격 · `eval_*` 하니스
- **패턴 차용(일부 갈라짐):** `retriever.py`(+하이브리드·필터) · `service.py`(+정밀도 게이트·핸드오프)
- **새로(이 프로젝트 정체):** Extractor · 매뉴얼 생성/검수/동결 · 핸드오프 축적 루프 · lineage 그래프 저장소
...
- **코드 파서:** Java/Spring · MyBatis XML · SQL (AST 수준 지향; 프로토타입은 regex였음)
...
1. ✅ **자산 형식 확정(프로토타입)** — 스키마=DDL+`COMMENT`(en+ko, 컬럼·코드값 권장), 코드=한글 주석 포함. 위 "데이터 자산" 참조.
2. **검증 슬라이스 1개** — 실 코드 화면 하나로 Extractor 정확도 + 매뉴얼 검수 통과 + 응답률 측정.
3. 슬라이스가 통과하면 → Extractor 일반화(프레임워크 어댑터) + 매뉴얼 생성/검수/적재 파이프라인.
4. `BNK_Bot` chat 도메인 이식 + 정밀도 게이트·핸드오프 루프 추가.
